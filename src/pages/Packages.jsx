...
const Packages = () => {
  ...
  if (!destination) return <h2 className="container">Destination not found!</h2>;
        ...
        <div className="images-grid">
          {destination.images.map((img, i) => (
            <div className="image-item" key={i}>
              <img src={img.src} alt={`${destinationName} ${i + 1}`} />
        ...
        <div className="card-grid">
          {destination.packages.map((pkg, i) => (
            <div key={i} className="card">
              ...
              <button className="btn" onClick={() => handleBookNow(pkg)}>Book Now</button>
            </div>
          ))}
        </div>
      </section>
